...
x_train = np.load('x_train_with_labels.npy', allow_pickle=True).astype('float32')
x_validation = np.load('x_validation_with_labels.npy', allow_pickle=True).astype('float32')
x_test = np.load('x_test_with_labels.npy', allow_pickle=True).astype('float32')

y_train = np.load('y_train_with_labels.npy', allow_pickle=True).astype('float32')
y_validation = np.load('y_validation_with_labels.npy', allow_pickle=True).astype('float32')
y_test = np.load('y_test_with_labels.npy', allow_pickle=True).astype('float32')
...
x_train = x_train.reshape(x_train.shape[0], 35, 24, 1)
x_validation = x_validation.reshape(x_validation.shape[0], 35, 24, 1)
x_test = x_test.reshape(x_test.shape[0], 35, 24, 1)
...
model = Sequential()
model.add(Conv2D(16, (3, 3), padding='valid', input_shape=(35, 24, 1), activation='relu', kernel_regularizer=l2(1e-06)))
